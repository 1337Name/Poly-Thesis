...
## How to use
First one needs to gather all the dependencies from requirements.txt file. Note that polydet does not have a pip package. It should be installed via the requirements.txt, alternatively it can be found at https://github.com/Polydet/polydet. Further, the Mitra tool is needed to get a baseline comparison with known techniques. It can be found at https://github.com/corkami/mitra (default path: `~/tools/mitra/mitra.py`, configure via `--mitra-path` parameter in generation/run_generation.py). ImageMagick is required for JPEGPixelGenerator, for this the `convert` command must be in PATH. In order to do the automated generation sample files are needed. 
